...
def has_budget(phone: str, chat_id: int, today: str, hour_key: str) -> bool:
...
def get_budget_status(phone: str, chat_id: int, today: str, hour_key: str) -> Dict[str, Any]:
    ...
    return {
        "account": {
            ...
            "percentage": (account_usage / PER_ACCOUNT_DAILY_CAP) * 100
        },
        "chat": {
            "used": chat_usage,
            "limit": PER_CHAT_HOURLY_CAP,
            "remaining": max(0, PER_CHAT_HOURLY_CAP - chat_usage),
            "percentage": (chat_usage / PER_CHAT_HOURLY_CAP) * 100
        },
        "has_budget": account_usage < PER_ACCOUNT_DAILY_CAP and chat_usage < PER_CHAT_HOURLY_CAP
    }

def consume_budget(phone: str, chat_id: int, n: int = 1, *, today: str = None, hour_key: str = None):
...
def load_all_state():
    global FLOOD_UNTIL, account_daily_usage, chat_hour_usage, state
